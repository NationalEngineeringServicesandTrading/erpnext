...
class TestAccountsController(FrappeTestCase):
	...
	def test_17_gain_loss_posting_date_for_normal_payment(self):
		...
		# Test payments with different exchange rates
		pe = (
			self.create_payment_entry(posting_date=adv_date, amount=1, source_exc_rate=75.1).save().submit()
		)
		...
		self.assertEqual(
			getdate(nowdate()),
			frappe.db.get_value("Journal Entry", exc_je_for_pe[0].parent, "posting_date"),
		)
		# Cancel Payment
		pe.reload()
		pe.cancel()
