...
class BlanketOrder(Document):
	...
	def upload_xml_file(self, file):
		upload_sales_order(self, file)
...
@frappe.whitelist()
def make_purchase_ordera(source_name):
	def update_item(source, target, source_parent):
		target_qty = source.get("qty") - source.get("ordered_qty")
		target.qty = target_qty if not flt(target_qty) < 0 else 0
		item = get_item_defaults(target.item_code, source_parent.company)
...
@frappe.whitelist()
def upload_sales_order(source_name, file):
	#doc = frappe.new_doc('File')
	#doc.insert()
	#_file = frappe.get_doc("File", {"file_url": doc.attach_file})
	doc = file #frappe.get_doc("File")
	#filename = doc.get_full_path()
	frappe.msgprint("Uploaded Successfully " + str(doc))
